...
function App() {
  ...
  React.useEffect(() => {
    fetch(encodeURI('cpxdevgame.azurewebsites.net/dsw/getready' + (localStorage.getItem('player') !== null ? '/'+ localStorage.getItem('player'): '')), {
      method: 'post', // or 'PUT'
      })
  ...
  const LoadReady = () => {
    ...
    loadstart = setInterval(() => {
      fetch(encodeURI('cpxdevgame.azurewebsites.net/dsw/checkreadygame' + (id !== null ? '/'+ id: '')), {
        method: 'post', // or 'PUT'
        headers: {
